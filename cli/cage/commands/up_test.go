...
func TestUp(t *testing.T) {
	t.Run("basic", func(t *testing.T) {
		...
		err := app.Run(context.Background(), []string{"cage", "up", "--region", "ap-notheast-1", "../../../fixtures"})
		assert.NoError(t, err)
	})
	t.Run("basic/ci", func(t *testing.T) {
		app, cagecli := setup(t, nil)
		cagecli.EXPECT().Up(gomock.Any()).Return(&types.UpResult{}, nil)
		err := app.Run(context.Background(), []string{"cage", "--ci", "up", "--region", "ap-notheast-1", "../../../fixtures"})
		assert.NoError(t, err)
	})
	t.Run("missing args", func(t *testing.T) {
		app, _ := setup(t, nil)
		err := app.Run(context.Background(), []string{"cage", "up", "--region", "ap-notheast-1"})
